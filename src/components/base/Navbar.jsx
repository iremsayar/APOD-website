  ...
  return (
    ...
    <nav className="container-fluid navbar navbar-expand-lg navbar-dark bg-dark border border-dark shadow-lg">
      <div className="col-sm-6 col-md-3">
        ...
        <div className="navbar-brand"> <Link to="/" className='text-decoration-none'><img src={logo} alt=" Logo" className='img-widht' />
          ...
          <li class="list-inline-item">
            {
              routes.filter(item => item.isNav).map((item, index) =>
                <Link key={index} className="text-light text-decoration-none px-2" to={item.path}>{item.title}</Link>)
            }
          </li>
      ...
      <div className=" col-md-3 ms-auto links d-none d-md-block">
        <div className="collapse navbar-collapse">
          <div className="navbar-nav">
            {
              routes.filter(item => item.isNav).map((item, index) =>
                <Link key={index} className="nav-item nav-link h4 text-light mx-4" to={item.path}>{item.title}</Link>)
            }
          </div>
        </div>
      </div> 
